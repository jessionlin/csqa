...
def make_mask(types, max_seq_length):
    ...
    max_type = max(types)
    # print("type is {}".format(types))
    
    diffs = []
    lc_0 = [l for l, t in enumerate(types) if t == 0]
    for i in range(max_type+1):
        loc = [l for l, t in enumerate(types) if t == i]
        one(mask, loc, loc)# 每个常识分句各部分彼此都可见

        if i != 0:
            half_one(mask, lc_0, loc) # 分支句子能看到主干句子所有内容
            diffs.append(loc)
    locs = [[],[]]
    for i in range(len(diffs)):
        locs[i%2].extend(diffs[i])
    one(mask, locs[0], locs[0])
    one(mask, locs[1], locs[1])
